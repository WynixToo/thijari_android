 ...
 *         {@link #load(Uri, OnLoadCompleteListener, int[])}. In this particular
...
public class PDFView extends SurfaceView {
	...
	private void drawPart(Canvas canvas, PagePart part) {
		...
		float localTranslation = toCurrentScale(part.getUserPage()
				* optimalPageWidth);
	...
	public void moveTo(float offsetX, float offsetY) {
		...
		if (isZooming()) {
			if (toCurrentScale(optimalPageWidth) < getWidth()) {
				...
				miniMapRequired = true;
				if (offsetX
						+ toCurrentScale(currentFilteredPage * optimalPageWidth) > 0) {
